...
ODR violations are invisible within a single translation unit.  
Each target here links one binary from two TUs that deliberately disagree,  
so a broken contract surfaces as a link error or as non-deterministic output rather than passing unnoticed.
...
- `typeinfo_bridge_{enabled,disabled}.cpp` - only one TU includes `<csl/ag/formatting/typeinfo.hpp>`.  
  ...
  the binary must link and behave deterministically. This is the fmt `<fmt/ranges.h>` opt-in contract.  
