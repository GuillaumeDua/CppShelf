...
namespace test::ag {
    ...
    // NOLINTEND(*-avoid-const-or-ref-data-members)
        ...
        using as_pack_t  = pack<T, const T, T&, const T &, T&&, const T&&>;
    };
    using type = S<int>;
}
...
namespace test::ag::std_tuple_element {
    consteval void check(){
        // std::tuple_element<std::size_t, T>
        []<std::size_t ... indexes>(std::index_sequence<indexes...>) constexpr {
            static_assert((std::same_as<
                std::tuple_element_t<indexes, std::remove_cvref_t<type>>,
                std::tuple_element_t<indexes, typename type::as_tuple_t>
            > and ...));
        }(std::make_index_sequence<csl::ag::size_v<std::remove_cvref_t<type>>>{});
    }
}
namespace test::ag::get_ {
    ...
    consteval void check_type_impl() {
        ...
        // std::get<T>
        []<typename ... Ts>(pack<Ts...>) constexpr {
            static_assert((std::same_as<
                decltype(std::get<Ts>(std::declval<T>())),
                decltype(std::get<Ts>(std::declval<typename std::remove_cvref_t<T>::as_tuple_t>()))
            > and ...));
        }(typename std::remove_cvref_t<T>::as_pack_t{});
    }
